package com.zhanke.flycotablayout;
...
import com.zhanke.flycotablayout.utils.FragmentChangeManager;
import com.zhanke.flycotablayout.utils.UnreadMsgUtils;
import com.zhanke.flycotablayout.widget.MsgView;
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void setCurrentTab(int currentTab) {
        if (mSpecialTab != null){
            for (int aMSpecialTab : mSpecialTab) {
                if (aMSpecialTab == currentTab) {
                    // 这里对特殊的tab处理
                    // 目前只是简单的屏蔽了原有的tab事件
                    return;
                }
